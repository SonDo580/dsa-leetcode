...
def simplify_path(path: str) -> str:
    ...
    # The first '/' (root directory) always remains on the stack
    ...
    for i in range(n):
        ...
        # [a]/[b]/..END -> [a]/END (the trailing '/' will be popped after the loop)
        # /../[b] -> //[b] (keep the root, the extra '/' will be skipped in the next iteration)
        if (
            stack[-1] == "."
            and stack[-2] == "/"
            and path[i] == "."
            ...
            # -> keep popping characters off the stack until:
            #    . encountering a second '/' (remove 1 '[segment]/')
            #    . OR the stack only has 1 character left (preserve the root)
            slash_removed = False
            while len(stack) > 1:
                if stack[-1] == "/":
                    if slash_removed:
...
def simplify_path(path: str) -> str:
    ...
    for segment in segments:
        # single period, consecutive slashes -> skip the segment
        if segment == "." or segment == "":
            continue

        # double period -> pop the latest segment
        if segment == "..":
            if len(stack) > 0:
                stack.pop()
            continue
