...
- Problem: Let G be an undirected graph. Check if G has a spanning tree.
  ...
  - To check if G is connected: perform DFS/BFS or use Union-Find. The final number of connected components must be 1.
...
- Problem: Find 1 minimum spanning tree of G.

## Cut property

- A `cut` is a partition of vertices in a graph into 2 disjoint subsets. A crossing edge is an edge that connects a vertex in 1 set with a vertex in the other set.
