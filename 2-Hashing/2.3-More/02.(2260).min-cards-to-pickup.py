...
Idea:
- Iterate over array and record the indices for every value in a hash map
- Iterate over each entry value (list of indices) to find the shortest distance.
"""
...
# ===== Improvement =====
"""
- Only need the last seen index of each value.
- At each item, we can calculate the min window length upto that point right away.
  . if value has not been seen -> no valid window (need a pair)
  . otherwise, check window from value's last index to current index.
"""
...
"""
Complexity:
1. Time complexity: still O(n), but only need 1 pass.
2. Space complexity: still O(n), but better on average.
   (worst case when there's no duplicates)
"""