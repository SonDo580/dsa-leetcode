...
-> Use a hashmap with sorted string as key.
   Each value will group the original strings.
"""
...
- Convert values of 'groups' to result: O(n)
=> Overall: O(n*m*log(m)) + O(n) = O(n*m*log(m))

2. Space complexity: 
- space for 'groups':
  . at most n entries, each key has m characters: O(n*m)
