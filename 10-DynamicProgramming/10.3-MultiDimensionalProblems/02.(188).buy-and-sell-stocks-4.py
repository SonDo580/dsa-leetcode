...
def max_profit(k: int, prices: list[int]) -> int:
    ...
    def dp(i: int, holding: bool, remain: int) -> int:
        ...
        if holding:  # can sell
            profit = max(profit, prices[i] + dp(i + 1, False, remain - 1))
        else:  # can buy
            profit = max(profit, -prices[i] + dp(i + 1, True, remain))
    ...
    for i in range(n - 1, -1, -1):
        for holding in range(2):
            for remain in range(1, k + 1):
                profit = dp[i + 1][holding][remain]  # profit if we skip
                if holding:  # can sell
                    profit = max(profit, prices[i] + dp[i + 1][0][remain - 1])
                else:  # can buy
                    profit = max(profit, -prices[i] + dp[i + 1][1][remain])
