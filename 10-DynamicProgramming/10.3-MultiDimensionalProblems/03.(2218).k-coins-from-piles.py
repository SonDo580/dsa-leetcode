...
- At the ith pile, we can skip or take some coins
  + skip: value = dp(i + 1, remain)
  + take: we can take up to jth coins from the pile
    (number of coins taken is j + 1):
    . value = sum(piles[i][0..j]) + dp(i + 1, remain - j - 1)
...
-> Recurrence relation:
   ...
   . take = max(sum(piles[i][:j + 1]) + dp(i + 1, remain - j - 1)
                for j in [0..min(remain, piles[i].length)-1]
- Base cases:
...
- Let n be the number of piles
      x be the average number of coins per pile

1. Time complexity:
- Number of states: n*k
- Each state has a for loop that iterates x times.
=> Overall: O(n*k*x)
...
1. Time complexity:
- Initialize dp: O(n*k)
- Fill up entries in dp: O(n*k*x)
=> Overall:  O(n*k) + O(n*k*x) = O(n*k*x)

2. Space complexity: O(n*k) for 'dp' matrix
"""
