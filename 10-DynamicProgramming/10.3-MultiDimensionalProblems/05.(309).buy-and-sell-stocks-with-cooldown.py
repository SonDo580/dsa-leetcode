...
- Identify required state variables:
  ...
  -> Let dp(i, holding, cooling_down) returns the maximum profit we can achieve from day i,
     holding indicates that we are holding stock,
     and cooling_down indicates that we are in the cool-down period.
  -> The answer is dp(0, holding=false, cooling_down=false)
- At each state:
  + We can always skip. The profit is:
    . dp(i, holding, cooling_down=false)
...
-> Recurrence relation:
   dp(i, holding, cooling_down) = max(skip, buy, sell) where:
...
- Base cases:
  + when i = prices.length, we can't make more transactions -> return 0
"""
    ...
    for i in range(n - 1, -1, -1):
        for holding in [0, 1]:
            for cooling_down in [0, 1]:
                ...
                if holding:  # can sell
                    max_profit = max(max_profit, prices[i] + dp[i + 1][0][1])
                elif not cooling_down:  # can buy
                    max_profit = max(max_profit, -prices[i] + dp[i + 1][1][0])
